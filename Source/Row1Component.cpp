...
public:
    OTTOSplashOverlay(const juce::Image& image, float displayTime,
        ...
        : splashImage(image), totalTime(displayTime),
          ...
          allowClickToClose(clickToClose)
    {
        setOpaque(true);
        
        // Only start timer for app startup splash (when clickToClose is false)
        // For OTTO logo splash (when clickToClose is true), timer should not auto-close
        if (!allowClickToClose) {
            startTimer(50);
        }
        
        // Set mouse cursor to indicate clickability for OTTO logo splash
        if (allowClickToClose) {
    ...
    {
        // Only close on click for OTTO logo splash screen
        if (allowClickToClose) {
            closeSplash();
        }
    }
...
void Row1Component::showSplashScreen() {
    ...
    if (splashImage.isValid()) {
        ...
        if (topLevelComponent) {
            // For OTTO logo splash, use a longer display time but allow click to close
            float splashTime = 10.0f; // Longer time, but user can click to close
            ...
            // Create the splash overlay with click-to-close enabled for OTTO logo splash
            auto splashOverlay = std::make_unique<OTTOSplashOverlay>(
                ...
                appBgColor,
                true  // Enable click-to-close for OTTO logo splash
            );
